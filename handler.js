    ...
    try {
        ...
        for (let name in global.plugins) {
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                if (xp > 200)
                    m.reply('Cheater -_-') // Hehehe
                else
                    m.exp += xp
                if (!isPrems && plugin.diamond && global.db.data.users[m.sender].diamond < plugin.diamond * 1) {
                    this.reply(m.chat, `✳️ Limit anda habis, silahkan beli melalui \n*${usedPrefix}buy* <amount> \n*${usedPrefix}buyall*`, m)
...
export async function participantsUpdate({ id, participants, action }) {
    ...
    switch (action) {
        ...
        case 'remove':
            ...
            break
        case 'promote':
            text = (chat.sPromote || this.spromote || conn.spromote || '@user It is now administrator')
        case 'demote':
            let pp = await this.profilePictureUrl(participants[0], 'image').catch(_ => 'https://i.ibb.co/1ZxrXKJ/avatar-contact.jpg') 
            if (!text)
                text = (chat.sDemote || this.sdemote || conn.sdemote || '@user is no longer administrator')
...
└─────────────
To deactivate this function, write 
*/off antidelete*
*.enable delete*
`.trim(), msg, {
